...
std::shared_ptr<GModel> GModel::CreateModelGObj(GModelType modelType, std::string modelPath, bool init_texture)
{
    auto gObj = std::make_shared<GModel>(modelType);
    GOBJModel gObjModel(modelType, modelPath);
    gObj->model = GGLModel::CreateWithObjModel(&gObjModel,init_texture);
    return gObj;
}

GModel::GModel(GModelType modelType)
    :GGameObject(GGameObjectType::kModel), modelType(modelType)
{
}
